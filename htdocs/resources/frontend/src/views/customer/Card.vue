  ...
  <div class="viewContainer" v-resize="onResize">
                      ...
                      <div v-if="card !== null && !loading">
                        ...
                        >
                          ...
                          <v-row
                            ...
                            class="mx-4"
                          >
                            <span class="body-1 font-weight-medium" v-html="formatNumber(card.balance.points)"/> <v-icon right size="18">mdi-coins</v-icon>
                          ...
                          <v-timeline dense clipped>
                            ...
                            >
                              <v-row justify="space-between" align="center">
                                <v-col>
                                  <strong>{{ $t(item.event) }} <span class="ml-2" :class="($store.state.app.dark) ? 'grey--text text--darken-2' : 'grey--text text--lighten-1'">{{ formatCurrency(item.purchase_amount, card.balance.currency) }}</span></strong>
                                  <div class="body-2"><span v-if="item.points > 0" v-html="'+'" />{{ formatNumber(item.points) }} {{ $t('points').toLowerCase() }}</div>
                                  <div class="mt-1 caption" v-if="item.event == 'purchase'" :class="($store.state.app.dark) ? 'grey--text text--darken-2' : 'grey--text text--lighten-1'">
                                ...
                                <v-col class="text-right caption" cols="4">{{ formatDate(item.created_at, 'ago') }}</v-col>
                              </v-row>
                            </v-timeline-item>
                          </v-timeline>
                        </v-card>
    ...
    <v-dialog v-model="dialogGenerateDiscountCode" persistent max-width="360">
      <ValidationObserver ref="generateDiscountCodeForm" v-slot="{ invalid }">
        ...
        >
          <v-card>
            <v-card-title class="headline">{{ $t('discount_code') }}
              <v-tooltip top max-width="340" color="NavBg NagFg--text">
                <template v-slot:activator="{ on }">
                  <v-icon style="margin-top:-2px;position: absolute; right: 15px" color="ctaBg" v-on="on">mdi-help-circle-outline</v-icon>
                </template>
            ...
            <div v-if="step == 1">
            <v-card-text>
              <p class="body-1">{{ $t('enter_discount_amount', { minAmount: formatCurrency(card.balance.rules.min_amount_redeemable, card.balance.currency), maxAmount: formatCurrency(card.balance.rules.max_amount_redeemable, card.balance.currency) }) }}</p>
              <v-row no-gutters>
                <v-col cols="3">
                  <v-btn
                    depressed 
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="secondary" class="no-caps" text @click="dialogGenerateDiscountCode = false">{{ $t('close') }}</v-btn>
              <v-btn color="NavBg NavFg--text" class="no-caps" depressed :loading="generateDiscountCodeForm.loading" :disabled="generateDiscountCodeForm.loading || invalid" type="submit">{{ $t('get_code') }} <v-icon right>mdi-arrow-right</v-icon></v-btn>
            </v-card-actions>
            </div>
            <div v-if="step == 2">
            <v-card-text>
              <p class="body-1">{{ $t('discount_code_details', { expires: generateDiscountCodeForm.expiresAt, points: generateDiscountCodeForm.calculatedPoints, amount: generateDiscountCodeForm.calculatedAmount, businessName: generateDiscountCodeForm.businessName }) }}</p>
              <div class="mt-5">
                <x-text-field
                  type="number"
              ...
              <qrcode v-if="!$vuetify.theme.dark" class="mt-0" :value="$init.config.schemeAndHost + '/business/redeem-points?c=' + generateDiscountCodeForm.code" :options="{ margin:0, width: qrWidth, errorCorrectionLevel: 'H', color: { light: '#ffffff00', dark: ($vuetify.theme.dark) ? '#ffffff' : '#000000' } }"></qrcode>
              <qrcode v-if="$vuetify.theme.dark" class="mt-0 elevation-1" :value="$init.config.schemeAndHost + '/business/redeem-points?c=' + generateDiscountCodeForm.code" :options="{ margin:8, width: qrWidth, errorCorrectionLevel: 'H', color: { light: '#ffffff', dark: '#000000' } }"></qrcode>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="NavBg NavFg--text" class="no-caps" depressed @click="step = 1; dialogGenerateDiscountCode = false">{{ $t('close') }}</v-btn>
            </v-card-actions>
            </div>
          </v-card>
        </v-form>
...
export default {
  ...
  computed: {
    qrWidth: function () {
      let w = parseInt(window.innerWidth) - 80
      return (w > 328) ? 328 : w
    }
  }
}
