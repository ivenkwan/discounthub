                      ...
                      <v-card-text v-else>
                        ...
                        <ValidationObserver ref="verifyCustomerNumberForm" v-slot="{ invalid }">
                          <v-form 
                            ...
                            lazy-validation
                            @submit.prevent="verifyCustomerNumber()"
                            autocomplete="off"
                            method="post"
                        ...
                        <p class="mt-0 mb-0 fg--text subtitle-1" v-if="supportsQrCamera()">{{ $t('issue_points_footer_qr') }}</p>
                        <p class="mt-0 mb-0 fg--text subtitle-1" v-if="!supportsQrCamera()">{{ $t('issue_points_footer') }}</p>
                      </v-card-text>
                    </v-card>
                  </v-col>
                </v-row>
...
export default {
  ...
  computed: {
    ...
    pointsIssued: function () {
      let points = this.business.points_per_currency * this.issuePointsForm.amount
