      ...
      <v-container class="inner-container">
        <v-row no-gutters>
          <div class="d-sm-none d-md-none d-lg-none d-xl-none d-flex align-center mr-3">
            <v-app-bar-nav-icon @click.stop="drawer = !drawer" color="NavFg"><v-icon>mdi-menu</v-icon></v-app-bar-nav-icon>
          </div>
          <div class="d-flex align-center">
            <router-link v-if="$init.app.logo != '' && $init.app.logo != null" :to="{name: 'home'}"><v-img :max-width="($vuetify.breakpoint.smAndDown) ? ((innerWidth < 400) ? '94' : '132') : '144'" :src="$init.app.logo"/></router-link>
            <router-link v-if="$init.app.logo == '' || $init.app.logo == null" :to="{name: 'home'}" :style="{ 'max-width' : ($vuetify.breakpoint.smAndDown) ? ((innerWidth < 400) ? '94px' : '132px') : '156px' }" :class="($vuetify.breakpoint.smAndDown) ? 'subtitle-2' : 'subtitle-1'" style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" class="font-weight-medium white--text text-decoration-none" v-html="$init.app.name"></router-link>
          </div>
          ...
          <v-toolbar-items class="d-flex align-center navigation" v-if="!$auth.check()">
            <v-btn
              ...
              :ripple="false"
              :to="{ name: 'login' }"
              class="no-caps subtitle-2 d-none d-sm-flex d-md-flex d-lg-flex d-xl-flex"
            >
              {{ $t('log_in') }}
            </v-btn>
          </v-toolbar-items>
          ...
          <div class="d-flex align-center ml-2" v-if="!$auth.check()">
            <v-btn color="NavFg NavBg--text" depressed tile :small="$vuetify.breakpoint.smAndDown" class="no-caps" :to="{name: 'register'}"><v-icon class="d-flex d-sm-none d-md-none d-lg-none d-xl-none">mdi-account</v-icon><span class="d-none d-sm-flex d-md-flex d-lg-flex d-xl-flex"> {{ $t('sign_up') }}</span></v-btn>
          </div>
        </v-row>
      </v-container>
    </v-app-bar>
      ...
      <v-container class="inner-container">
        ...
        >
          ...
          >
            ...
            <ul class="nav-list">
              <li><router-link :to="{name: 'home'}" class="NavFg--text text-decoration-none">{{ $t('home') }}</router-link></li>
              <li><router-link :to="{name: 'about'}" class="NavFg--text text-decoration-none">{{ $t('about') }}</router-link></li>
              <li v-if="!$auth.check()"><router-link :to="{name: 'login'}" class="NavFg--text text-decoration-none">{{ $t('log_in') }}</router-link></li>
              <li v-if="!$auth.check()"><router-link :to="{name: 'register'}" class="NavFg--text text-decoration-none">{{ $t('sign_up') }}</router-link></li>
              <li v-if="$auth.check() && $auth.user().role == 1"><router-link :to="{name: 'admin.dashboard'}" class="NavFg--text text-decoration-none">{{ $t('dashboard') }}</router-link></li>
              <li v-if="$auth.check() && $auth.user().role == 2"><router-link :to="{name: 'customer.earn'}" class="NavFg--text text-decoration-none">{{ $t('dashboard') }}</router-link></li>
              <li v-if="$auth.check() && $auth.user().role == 3"><router-link :to="{name: 'business.issue-points'}" class="NavFg--text text-decoration-none">{{ $t('dashboard') }}</router-link></li>
              <li v-if="$auth.check() && $auth.user().role == 4"><router-link :to="{name: 'staff.dashboard'}" class="NavFg--text text-decoration-none">{{ $t('dashboard') }}</router-link></li>
            </ul>
          </v-col>
          <v-col
            cols="6"
          ...
          >
            ...
            <ul class="nav-list">
              <li><router-link :to="{name: 'terms'}" class="NavFg--text text-decoration-none">{{ $t('terms_and_conditions') }}</router-link></li>
              <li><router-link :to="{name: 'privacy'}" class="NavFg--text text-decoration-none">{{ $t('privacy_policy') }}</router-link></li>
            </ul>
          </v-col>
          <v-col
            cols="6"
          ...
          >
            ...
            <ul class="nav-list">
              <li><router-link :to="{name: 'contact'}" class="NavFg--text text-decoration-none">{{ $t('contact_us') }}</router-link></li>
            </ul>
          </v-col>
          <v-col
            cols="6"
