  ...
  <div class="fill-height">
    ...
    <v-card flat v-show="loaded" color="transparent">
      ...
      <div v-if="!filteredResults && !loading && loaded && rows.length == 0" class="text-center my-10">
        ...
        <div>
          <v-btn @click="uuid = null; dataForm = true" v-if="settings.create === true" depressed large class="mt-3 mb-5 no-caps" color="ctaBg ctaFg--text"><v-icon left>mdi-plus</v-icon> {{ translations.create_item }}</v-btn>
        </div>
      </div>
