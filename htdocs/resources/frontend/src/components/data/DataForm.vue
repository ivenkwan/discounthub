<template>
  <v-card>
    ...
    <div v-if="!loadingForm && limitReached">
      <v-card-text class="pt-0">
        <p class="subtitle-1">
          <span v-if="max != 0">You have reached the maximum of {{ max }} {{ translations.items_lowercase }}. </span>
          <i18n v-if="max == 0" path="premium_account_required">
            <template #premium_account>
              <router-link :to="{name: 'settings.business.subscription'}">{{ $t('premium_account').toLowerCase() }}</router-link>
            </template>
          </i18n></p>
      </v-card-text>
    </div>
  ...
  <!--              </div>-->
        ...
        <v-card-actions v-if="!loadingForm">
          <v-spacer></v-spacer>
          <v-btn color="grey" text :disabled="form.loading" large @click="$emit('data-list-events', {'closeDialog': true, 'reload': false})" class="no-caps">{{ $t('close') }}</v-btn>
          <v-btn color="ctaBg ctaFg--text" depressed :loading="form.loading" :disabled="form.loading || invalid"  large type="submit" class="no-caps">{{ (uuid === null) ? $t('create') : $t('save') }}</v-btn>
        </v-card-actions>
      </v-form>
    </ValidationObserver>
